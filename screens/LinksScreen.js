...
import { Input, Label, Icon, Container, Header, Content, Accordion, ListItem, CheckBox, Body, Text, Button, ActionSheet, Item, Form, Thumbnail, DatePicker} from "native-base";
const dataArray = [
...
const sheetButons = ["Inspection Sheet", "Soil Sample", "Dust Sample", "Job Completion Checklist", "Property Description Checklist", "Form 5.0", "Layout Photo", 'Cancel'];
...
const CANCEL_INDEX = 10;
const CANCEL_INDEX2 = 7;

function InsSheet(props){
  console.log('LENGTH', props.data.length -1)
  return(
    ...
    <Item>
      ...
      <Accordion dataArray={props.data} sheetId={props.id} expanded={props.data.length - 1} removeWindow={props.removeWindow} renderHeader={props.renderWindowHeader} renderContent={(content) => props.renderSheet(content, props.id)}/>
      <Button
       onPress={() =>
       ActionSheet.show(
         {
...
_renderWindowHeader(obj, expanded) {
  return (
    <View
      style={{ flexDirection: "row", padding: 10, justifyContent: "space-between", alignItems: "center", backgroundColor:obj.done?"#5FB49C":"#DEEFB7" }}
    >
    ...
    return (
        ...
        ActionSheet.show(
          ...
          <Grid style={{marginTop:10}}>
            <Col style={{ backgroundColor: '#E1E5F2', height: 250 }}>
              <Text>Sill</Text>
              <Button
               onPress={() =>
               ActionSheet.show(
             ...
             <Col style={{ backgroundColor: '#FFFFFF', height: 250 }}>
             <Text>Sash</Text>
             <Button
              onPress={() =>
              ActionSheet.show(
        ...
        <Button block error style={{marginTop:10, marginBottom:10}} onPress={() => {
          this.state.insSheets.map(x => {
            if(x.id == content.id){
              x.expanded=false
              console.log('setting expanded to false', x);
